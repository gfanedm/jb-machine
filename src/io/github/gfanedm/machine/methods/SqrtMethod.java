...
public class SqrtMethod extends Method {
	...
	public SqrtMethod(int code) {
		super(code);
	}

	@Override
	public void executeFunction(MemoryHandler memoryHandler, Instruction instruction) {
		Machine.getMachine().getPipelineHandler().getMethod(3).executeFunction(memoryHandler,
				new Instruction(0, instruction.get(1), instruction.get(1), instruction.get(3)));
	}
